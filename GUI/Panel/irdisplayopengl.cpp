...
IRDisplayOpenGL::IRDisplayOpenGL(QWidget *parent)
...
QSize IRDisplayOpenGL::sizeHint() const
{
...
QSize IRDisplayOpenGL::minimumSize() const
{
    int minW = 250;
...
int IRDisplayOpenGL::heightForWidth(int width) const
{
    return qRound(width * ASPECT_RATIO);
}

/* Handle mouse move events for hover detection */
void IRDisplayOpenGL::mouseMoveEvent(QMouseEvent *event)
{
    //return;
...
void IRDisplayOpenGL::leaveEvent(QEvent *event)
{
    Q_UNUSED(event);
    hoveredTargetIndex = -1;
    update();
...
IRDisplayOpenGL::Angles IRDisplayOpenGL::vectorToAngles(
    const float &x,
    const float &y,
    const float &z)
{
...
/* Select entity for display */
void IRDisplayOpenGL::selectEntity(Entity* entit)
{
    entity = nullptr;
    id = "";
    ...
    for (auto const& pair :  *entity->sensors->sensors) {
        ...
        if (s && s->subType == Sensor::SubType::IR) {
            sensor = s;
...
void IRDisplayOpenGL::RemoveEntity(QString ID)
{
    if (id == ID) {
...
void IRDisplayOpenGL::updateRadar()
{
    if (entity && sensor) {
...
void IRDisplayOpenGL::paintEvent(QPaintEvent * /*event*/)
{
    QElapsedTimer timer;
...
void IRDisplayOpenGL::drawBackground(QPainter &p)
{
    p.save();
    // Fill background
...
void IRDisplayOpenGL::drawVanishingPoint(QPainter &p, const QPoint &center)
{
    p.save();
...
void IRDisplayOpenGL::drawEntity(QPainter &p,const EOImageType &eoImageType, const QPoint &position, float size, int heading =0,int pitch = 0)
{
    p.setPen(QPen(QColor(255, 105, 180)));
    p.setBrush(Qt::NoBrush);
...
void IRDisplayOpenGL::drawTargetDetection(QPainter &p, const QPoint &center, const QSize &size)
{
...
void IRDisplayOpenGL::drawEntity(QPainter &p, const EOImageType &eoImageType, const QPoint &position, float relativeRadius, float size, int heading, int pitch)
{
    p.setPen(QPen(QColor(255, 105, 180)));
    p.setBrush(Qt::NoBrush);
...
void IRDisplayOpenGL::drawHorizon(QPainter &p, const QPoint &center)
{
    p.save();
    p.setPen(radarGreen);
...
void IRDisplayOpenGL::loadMultiDirectionalImages(QPainter &p, int heading, int pitch)
{
...
void IRDisplayOpenGL::drawCenterMark(QPainter &p, const QPoint &center)
{
    p.save();
...
void IRDisplayOpenGL::drawConcentricCircles(QPainter &p, const QPoint &center, int outerRadius)
{
    p.save();
...
void IRDisplayOpenGL::drawTicksAndLabels(QPainter &p, const QPoint &center, int outerRadius)
{
    p.save();
...
void IRDisplayOpenGL::drawTopMarker(QPainter &p, const QPoint &center, int outerRadius)
{
    p.save();
...
void IRDisplayOpenGL::drawTargetAndPath(QPainter &painter)
{
    int w = width();
    int h = height();
    int centerX = w/2;
...
void IRDisplayOpenGL::drawCropRectangle(QPainter &p, const int &h, const int &w)
{
    p.save();
...
void IRDisplayOpenGL::drawRadarRing(QPainter &p, const QPoint &center, int outerRadius)
{
    p.save();
...
void IRDisplayOpenGL::drawDisplayScreen(QPainter &p, const QPoint &center, const int &screenHeight, const int &screenWidth)
{
    p.save();
...
void IRDisplayOpenGL::clearDisplayScreenSurrounding(QPainter &p, const int &screenHeight, const int &screenWidth)
{
    p.save();
...
float IRDisplayOpenGL::ratioOfDimension(const QSize &screenDimension)
{
    float ratio = static_cast<float>(screenDimension.height())/static_cast<float>(screenDimension.width());
    return ratio;
}

QSize IRDisplayOpenGL::relativeHeightNWidth(
...
void IRDisplayOpenGL::debug(const QString &str, const debugEODisplay &currentdebugType)
{
...
bool IRDisplayOpenGL::dbgIsAllow(const debugEODisplay &currentdebugType)
{
