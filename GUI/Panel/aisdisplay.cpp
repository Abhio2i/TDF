...
AISDisplay::AISDisplay(QWidget *parent)
...
QSize AISDisplay::sizeHint() const
{
...
QSize AISDisplay::minimumSize() const
{
    int minW = 250;
...
int AISDisplay::heightForWidth(int width) const
{
...
void AISDisplay::leaveEvent(QEvent *event)
{
    Q_UNUSED(event);
    hoveredTargetIndex = -1;
    update();
...
void AISDisplay::selectEntity(Entity* entit)
{
    entity = nullptr;
    id = "";
...
void AISDisplay::RemoveEntity(QString ID)
{
    if (id == ID) {
...
void AISDisplay::paintEvent(QPaintEvent * /*event*/)
{
    QElapsedTimer timer;
    ...
    // Profiler::currentFrame->AISDisplay = elapsedMs;
}

// %%% Drawing Methods %%%
/* Draw display background */
void AISDisplay::drawBackground(QPainter &p)
{
    p.save();
    // Fill background
...
void AISDisplay::drawRadarRing(QPainter &p, const QPoint &center, int outerRadius)
{
    p.save();
...
void AISDisplay::drawConcentricCircles(QPainter &p, const QPoint &center, int outerRadius)
{
    p.save();
...
void AISDisplay::drawTicksAndLabels(QPainter &p, const QPoint &center, int outerRadius)
{
    p.save();
...
void AISDisplay::drawCenterMark(QPainter &p, const QPoint &center)
{
    p.save();
...
void AISDisplay::drawTopMarker(QPainter &p, const QPoint &center, int outerRadius)
{
    p.save();
...
void AISDisplay::drawTargetAndPath(QPainter &painter)
{
    int w = width();
    int h = height();
    int centerX = w/2;
