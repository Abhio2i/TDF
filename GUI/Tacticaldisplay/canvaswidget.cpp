...
//     painter.translate(width() / 2.0, height() / 2.0); // Center the scaling
//     painter.scale(zoomLevel, zoomLevel); // Apply zoom
//     painter.translate(-width() / 2.0, -height() / 2.0); // Restore origin
//     painter.translate(canvasOffset); // Apply panning offset
...
//     if (isDrawingTrajectory && !currentTrajectory.empty()) {
//         painter.save();
//         QPen pen(Qt::magenta, 2 / zoomLevel, Qt::DashLine); // Adjust pen width for zoom
//         painter.setPen(pen);
//         painter.setBrush(Qt::NoBrush);
//         QPolygonF polyline;
//         for (const Waypoints* waypoint : currentTrajectory) {
//             // Scale waypoint positions
//             polyline << QPointF(waypoint->position->x, waypoint->position->y);
//             painter.setBrush(Qt::magenta);
//             painter.drawEllipse(QPointF(waypoint->position->x, waypoint->position->y), 3 / zoomLevel, 3 / zoomLevel);
//             painter.setBrush(Qt::NoBrush);
//         }
//         if (polyline.size() > 1) {
//             painter.drawPolyline(polyline);
...
//     painter.setBrush(Qt::green);
//     painter.setPen(Qt::NoPen);
//     painter.drawEllipse(QPointF(width() / 2.0, height() / 2.0), 10 / zoomLevel, 10 / zoomLevel);

//     painter.setPen(QPen(Qt::red, 2 / zoomLevel));
//     float radius = qMin(width(), height()) / (2.5 * zoomLevel);
//     float rad = qDegreesToRadians(angle);
//     QPointF center(width() / 2.0, height() / 2.0);
//     QPointF endpoint(center.x() + radius * cos(rad), center.y() - radius * sin(rad));
//     painter.drawLine(center, endpoint);
...
//     if (showXGrid) {
//         painter.setPen(QColor(128, 128, 128, alpha));
//         for (float x = 0; x < width() / zoomLevel; x += 20 / zoomLevel) {
//             painter.drawLine(x, 0, x, height() / zoomLevel);
//         }
//     }

//     if (showYGrid) {
//         painter.setPen(QColor(128, 128, 128, alpha));
//         for (float y = 0; y < height() / zoomLevel; y += 20 / zoomLevel) {
//             painter.drawLine(0, y, width() / zoomLevel, y);
//         }
//     }
// }
...
//             float x = point->x * entry.size->x / zoomLevel; // Scale size
//             float y = point->y * entry.size->y / zoomLevel;
...
//         painter.setPen(QPen(*mesh->color, mesh->lineWidth / zoomLevel));
//         painter.setBrush(*mesh->color);
...
// //     // Map GISlib zoom levels (1 to 19) to CanvasWidget zoom factors
// //     // Example: GISlib zoom 1 -> zoomLevel 0.5, zoom 19 -> zoomLevel 2.0
// //     zoomLevel = 0.5f + (zoom - 1) * (1.5f / 18.0f); // Linear mapping from zoom 1-19 to zoomLevel 0.5-2.0
// //     Console::log("Canvas zoom level set to: " + std::to_string(zoomLevel));
// //     update();
// // }
// void CanvasWidget::onGISZoomChanged(int zoom) {
//     zoomLevel = 0.5f + (zoom - 1) * (1.5f / 18.0f); // Linear mapping from GISlib zoom 1-19 to CanvasWidget zoomLevel 0.5-2.0
//     Console::log("Canvas zoom level set to: " + std::to_string(zoomLevel));
//     update();
// }
