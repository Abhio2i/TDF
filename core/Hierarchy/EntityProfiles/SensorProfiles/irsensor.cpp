...
// Precompute constants at compile-time using constexpr and 'f' suffix for floats
constexpr float EARTH_RADIUS_F = 6378137.0f;
constexpr float PI_F           = 3.141592653589793f;
constexpr float DEG_TO_RAD_F   = PI_F / 180.0f; // Pre-calculated multiplier


IRSensor::IRSensor(Hierarchy* h) : Sensor(h) {
    subType = SubType::IR;
    // azimuth = 360;
    frequency = 10.0f;
    eo_ir = new EO_IR(h);
...
Surrounding IRSensor::getSurrounding(
    float radius,
    float atmCoeff,
    float rainRate,
    float fog,
...
float IRSensor::getVeticalAngle(std::pair<int, int> resolution)
{
...
double IRSensor::deg2rad(double degrees) {
    return degrees * PI / 180.0;
}
// Helper function to convert degrees to radians
double IRSensor::degToRad(double degrees) {
    return degrees * M_PI / 180.0;
}
// Helper: Geodetic (Lat, Lon, Alt) to ECEF (X, Y, Z)
void IRSensor::GeodeticToECEF(double lat, double lon, double alt, double &X, double &Y, double &Z) {
...
PoseOpenGL IRSensor::CalculateRelativeOpenGLPose(const PoseGeo& target, const PoseGeo& sensor) {
 ...
 * +X = East, +Y = Up, -Z = North
 */
CartesianCoord IRSensor::getRelativeCoordinatesGL(GeoCoord sensor, GeoCoord target) {
...
bool IRSensor::isUnderRangeNAngle(
...
float IRSensor::calculateDetectionProbability(
...
ScreenTarget IRSensor::getTargetScreenBoundingBox(
    Resolutions res,
...
QJsonObject IRSensor::toJson() const {
    QJsonObject obj;
    obj["active"] = Active;
...
void IRSensor::fromJson(const QJsonObject& obj) {
    if (obj.contains("id")){
...
void IRSensor::setEnvironment(float relativeHumidity, float absoluteHumidity, float rainfallRate, float snowfallEquivalent, float ambientTemp, float backgroundTemp, float aerosolConcentration, float baseExtinctionCoeff, float ambientIlluminance, float solarIrradiance)
{
...
void IRSensor::setEnvironmentToDefault()
{
...
void IRSensor::debug(const QString &str, const debugSQLite &currentdebugType)
{
...
bool IRSensor::dbgIsAllow(const debugSQLite &currentdebugType)
{
