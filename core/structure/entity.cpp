...
Entity::Entity(Hierarchy* h) {
    ...
    if (hierarchy) {
        ...
        //emit hierarchy->entityAddedPointer(QString::fromStdString(parentID),this);
        //emit hierarchy->entityAdded(QString::fromStdString(parentID),QString::fromStdString(ID),QString::fromStdString(Name));
    }
    ...
    //emit hierarchy->meshAdded(QString::fromStdString(ID),QString::fromStdString(Name),transform,meshRenderer2d->Meshes);
    // componentMap = {
    //     {"transform", [this]() {
    ...
    //      }},
    //     {"dynamicModel", [this]() {
    //          if (!dynamicModel) dynamicModel = new DynamicModel();
    //          dynamicModel->transform = transform;
    //          dynamicModel->rigidbody = rigidbody;
    //      }},
    //     {"meshRenderer2d", [this]() {
    //          if (!meshRenderer2d) meshRenderer2d = new MeshRenderer2D();
    //      }}
    // };
}
...
void Entity::spawn(){
    ...
    //emit parent->meshAdded(QString::fromStdString(ID),QString::fromStdString(Name),transform,meshRenderer2d->Meshes);

    // if(type == Constants::EntityType::Radio)
    //     addComponent("radios");
    // else
    // if(type == Constants::EntityType::Weapon)
    //     addComponent("weopons");
    // else
    // if(type == Constants::EntityType::Radio)
    //     addComponent("radios");
...
std::vector<Component*> Entity::getSupportedComponents(){
    ...
    components.push_back(new MeshRenderer2D());
    components.push_back(new DynamicModel());
    components.push_back(new Rigidbody());
    components.push_back(new Collider());
...
QJsonObject Entity::getComponent(std::string name){
    if(name =="transform"){
...
void Entity::updateComponent(QString name,const QJsonObject& obj){
    if(name =="transform"){
...
QJsonObject Entity::toJson(){
    ...
    obj["active"] = Active;
    // obj["type"] = static_cast<int>(type); // enum as int
    ...
    entityObj["value"] = entityTypeToString(type); // "box"
    obj["type"] = entityObj;
    return obj;
}
